...
if poolId is not None:
    ...
    if epoch is not None:
        print("Checking leadership for Epoch", epoch)
        url=("https://api.crypto2099.io/v1/sigma/"+str(poolId)+"/"+str(epoch))
    else:
        print("No epoch provided, using latest known epoch.")
        url=("https://api.crypto2099.io/v1/sigma/"+str(poolId))
    try:
        page = urlopen(url)
        pool_data = json.loads(page.read().decode("utf-8"))
        # print(pool_data) # debug
    except:
        print("Error unable to fetch data from the sigma API.")
        exit()
    try:
        ...
        print("Error one or more data points from the API are missing or invalid. Please try again.")
        parse.format_help()
        parser.print_help()
        exit()
    ...
    try:
        ...
        print("Error unable to fetch data from the epoch API.")
    ...
    try:
        ...
        print("Error one or more arguments are missing or invalid. Please try again.")
        parser.format_help()
        parser.print_help()
        exit()
...
if eta0 == 'TBD':
        print("Error you're a bit early. The epoch nonce for Epoch",epoch,"isn't ready yet! Try again later.")
        exit()
...
if platform == "linux" or platform == "linux2":
    ...
    libsodium = cdll.LoadLibrary("/usr/local/lib/libsodium.so.23")
...
for slot in range(firstSlotOfEpoch,epochLength+firstSlotOfEpoch):
    ...
    if slotLeader:
        ...
        if overlaySlot:
            stolencount+=1
            print("\nLookup result: " + timestamp.strftime('%Y-%m-%d %H:%M:%S') + " ==> Stolen by BFT for " + str(slot-firstSlotOfEpoch) + ", Cumulative stolen blocks due to d param: " + str(stolencount))
        else:
            slotcount+=1
            print("\nLookup result: " + timestamp.strftime('%Y-%m-%d %H:%M:%S') + " ==> Leader for " + str(slot-firstSlotOfEpoch) + ", Cumulative epoch blocks: " + str(slotcount))

if slotcount == 0:
    print("\nLookup result: No slots found for current epoch... :(")
if overlaySlot:
    print("\nLookup result: Slots stolen by BFT nodes: " + str(stolencount))
